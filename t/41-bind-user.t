...
use Net::LDAP::Constant qw/LDAP_SUCCESS LDAP_INVALID_DN_SYNTAX LDAP_INVALID_CREDENTIALS/;
...
test_requests(
    ...
    requests_sub => sub {
        ...
        $mesg = $ldap->bind( 'CN=Alexei Znamensky,DC=SnakeOil,DC=com',
            password => 'potatoes' );
        is( $mesg->code, LDAP_SUCCESS, 'result: ' . $mesg->error_desc ) || diag explain $mesg;
        $mesg = $ldap->unbind;
        ...
        is( $mesg->code, LDAP_INVALID_CREDENTIALS, 'result: ' . $mesg->error_desc ) || diag explain $mesg;
        $mesg = $ldap->unbind;
        is( $mesg->code, LDAP_SUCCESS,
            'should be able to unbind (' . $mesg->error_desc . ')' );

        $ldap = ldap_client();
        $mesg = $ldap->bind( 'caspertheghost', password => 'dontneednopassword' );
        is( $mesg->code, LDAP_INVALID_DN_SYNTAX, 'result: ' . $mesg->error_desc ) || diag explain $mesg;
        $mesg = $ldap->unbind;
        is( $mesg->code, LDAP_SUCCESS,
            'should be able to unbind (' . $mesg->error_desc . ')' );
