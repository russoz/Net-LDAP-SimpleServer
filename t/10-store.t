...
is( scalar(@{$list}), 3, 'the list contains three elements' ) || diag explain $list;
...
subtest 'list_with_dn_scope on entry dn' => sub {
    ...
    ok($list_baseobj, 'returns a list with dn and scope baseobj') || diag 'list baseobj';
    is(scalar(@{$list_baseobj}), 1, 'list_baseobj contains one element') || diag explain $list_baseobj;
    is($list_baseobj->[0]->dn(), $TEST_DN);

    my $list_onelevel = $store->list_with_dn_scope($TEST_DN, SCOPE_ONELEVEL);
    ok($list_onelevel, 'returns a list with dn and scope onelevel') || diag 'list onelevel';
    is(scalar(@{$list_onelevel}), 1, 'list_onelevel contains one element') || diag explain $list_onelevel;
    is($list_baseobj->[0]->dn(), $TEST_DN);

    my $list_subtree = $store->list_with_dn_scope($TEST_DN, SCOPE_SUBTREE);
    ok($list_subtree, 'returns a list with dn and scope subtree') || diag 'list subtree';
    is(scalar(@{$list_subtree}), 1, 'list_subtree contains one element') || diag explain $list_subtree;
    is($list_baseobj->[0]->dn(), $TEST_DN);
};
...
subtest 'list_with_dn_scope on second level dn' => sub {
    ...
    ok($list_baseobj, 'returns a list with dn and scope baseobj') || diag '2list baseobj';
    is(scalar(@{$list_baseobj}), 1, 'list_baseobj contains one element') || diag explain $list_baseobj;
    is($list_baseobj->[0]->dn(), $DN_LEVEL2);

    my $list_onelevel = $store->list_with_dn_scope($DN_LEVEL2, SCOPE_ONELEVEL);
    ok($list_onelevel, 'returns a list with dn and scope onelevel') || diag '2list onelevel';
    is(scalar(@{$list_onelevel}), 2, 'list_onelevel contains two element') || diag explain $list_onelevel;
    cmp_deeply( [map {$_->dn()} @{$list_onelevel}], set($DN_LEVEL2, $TEST_DN) );

    my $list_subtree = $store->list_with_dn_scope($DN_LEVEL2, SCOPE_SUBTREE);
    ok($list_subtree, 'returns a list with dn and scope subtree') || diag '2list subtree';
    is(scalar(@{$list_subtree}), 2, 'list_subtree contains two element') || diag explain $list_subtree;
    cmp_deeply( [map {$_->dn()} @{$list_onelevel}], set($DN_LEVEL2, $TEST_DN) );
};
